...
class Room extends View {
    ...
    private generateDoors = () => {
        ...
        for (let i = 0; i < 4; i++) {
            ...
            if (this.random(1, 5) % 2 === 0 && onlyOneSensitiveData === false) {
                ...
                this.doors.push(
                    new Door(this.sensitiveData[this.random(0, this.sensitiveData.length, alreadyUsedValues)], position[i], true, false, this.canvas)
                );
            } else {
                ...
                if (i === 3 && onlyOneSensitiveData === false) {
                    this.doors.push(
                        new Door(
                            this.sensitiveData[this.random(0, this.sensitiveData.length, alreadyUsedValues)],
                            position[i],
                            true,
                            false,
                            this.canvas
                    ...
                    const index = this.random(0, this.nonSensitiveData.length, alreadyUsedValues);

                    alreadyUsedValues.push(index);

                    this.doors.push(new Door(this.nonSensitiveData[index], position[i], false, false, this.canvas));
                }
            }
        }
    };
     ...
     */
    private random = (min: number, max: number, alreadyUsedValues: number[] = [-1]) => {
